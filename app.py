...
st.caption("上传 1~2 个 Excel/CSV → 智能查询 或 指定索引+字段做数据校对 → 返回结果预览 + Excel 公式示例")
...
with tab_check:
    ...
    if df2 is None:
        ...
        st.markdown(
            "思路：指定索引列匹配两表记录，再指定要校对的字段映射，对比两侧值是否一致。"
        )
        ...
        # 2) 要校对字段映射
        st.caption(
            "要校对的字段：可以直接写同名列（如：未税金额），"
            "也可以写“表1列=表2列”（例如：开票数量=数量）。多个用逗号分隔。"
        )
        ...
        if go_check:
            ...
            if not keys or not_in_1 or not_in_2:
                ...
                if not compare_pairs:
                    st.error("没有解析出有效的校对字段，请检查列名/映射是否正确。")
                else:
                    # ---- 表2重命名并合并 ----
                    left = df1.copy()
                    right = df2.copy()
                    ...
                    if mismatch_details:
                        st.markdown("### ❌ 指定字段不一致明细（前 200 条）")
                        for col, diff in mismatch_details:
                            st.markdown(f"**字段：{col}**")
                            st.dataframe(diff, width="stretch")
                    else:
                        st.markdown("### ✅ 在匹配行中，指定字段全部一致（基于当前索引和映射）")

                    # ---- 生成 Excel 对账公式示例 ----
                    example_cols = ", ".join(compare_cols[:5]) if compare_cols else ""
...
要求：
- 用简短中文解释整体思路。
- 使用 XLOOKUP 或 VLOOKUP + IF 等写法。
- 假设索引列在两表的前几列，使用绝对列引用，例如 Sheet2!$A:$A。
输出严格 JSON：
{{
